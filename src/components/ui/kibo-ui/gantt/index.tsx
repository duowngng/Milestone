...
const getWidth = (
  ...
  if (context.range === "daily") {
    const delta = differenceIn(endAt, startAt) + 1;
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({
  ...
  const duration = tempEndAt
    ? formatDistance(feature.startAt, addDays(tempEndAt, 1))
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <div
              className="pointer-events-auto h-2 w-2 shrink-0 rounded-full"
              style={{
          ...
          >
            <p className="text-sm white font-medium">
              {snakeCaseToTitleCase(feature.status.name)}
            </p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative z-10 flex h-full w-full items-center justify-between gap-2 text-left",
...
export const GanttFeatureItem: FC<GanttFeatureItemProps> = ({
  ...
  return (
    ...
    >
      <div
        className="pointer-events-auto absolute top-0.5 transition-all duration-300 ease-out"
        style={{
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  useEffect(() => {
    if (scrollRef.current) {
      ...
      const differenceInFunc = getDifferenceIn(range);
      const offset = differenceInFunc(today, timelineStartDate);
      const innerOffset = calculateInnerOffset(
        today,
        range,
      ...
      const todayPosition = ((columnWidth * zoom) / 100) * offset + innerOffset;

      scrollRef.current.scrollLeft =
        todayPosition - scrollRef.current.clientWidth / 2 + sidebarWidth;
      setScrollX(scrollRef.current.scrollLeft);
    }
  }, [range, zoom, setScrollX, timelineData, columnWidth, sidebarWidth]);
